...
public class OkHttpUtils {
    ...
    /** 信任的 host, 仅在需要时配置 */
    private static final HashSet<String> TRUSTED_HOSTS = null;
...
//            .hostnameVerifier(new StandardHostnameVerifier(trustedHosts))
    ...
    private static final OkHttpClient SSE_CLIENT = new OkHttpClient.Builder(DEFAULT_CLIENT)
            // 超时时间
...
// ------------------------------ Post 访问 ------------------------------
    ...
    public static void ssePostJson(@NonNull String url, String bodyJsonStr, EventSourceListener eventSourceListener) {
        Request request = createPostRequest(url, bodyJsonStr, null, null);
        EventSource.Factory factory = EventSources.createFactory(SSE_CLIENT);
        // 创建事件
